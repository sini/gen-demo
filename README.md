...
| C13 -- `foldLayers`                                        | 0017                     | `genAlgebra.record.foldLayers`, all three strategies plus the default channel in one call; `foldNestedLayers` over a literal dotted key `"a.b"` and the nested path `a.b` it spells keeps both as two leaves; over a shape conflict the last layer providing `a.b` wins, a scalar resetting the subtree before it and a later subtree merging onto the reset                                                                                                                                                                                                                                                                                            |
...
| C21 -- the relocation preserves identity content           | 0016 ruling 7, 0033      | the corpus's `thimble` composed the retired way (`imports = [ config.schema.hank ]`) and the relocated way (`inherits = [ "hank" ]`, gen-schema's staged `evalSchema`) preserves the kind's identity CONTENT: the relocated instance recomputed under the retired kind is the retired instance, over one key set; the stamp follows `kindEq`. Dropping the inheritance MOVES the key set, which is what makes the equality non-vacuous                                                                                                                                                                                                                  |
| C22 -- a bounded extent peer-read                          | 0026                     | gen-bind's `mkHostedTerminal` adapter over a real `genDelivery.realize`; one node carries an invented mark admitting no label, and its handed `specialArgs.nodes` is bounded to empty while the mark is named on every withheld peer                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| C56 -- the three identity regimes                          | 0034                     | `identity-regimes`: through the hub's mint, `whipstitch {thread=madder}` built twice mints ONE identity, `woad` another and a second `revision` a third, all named `whipstitch`; a sealed pair decides past a refusing `__id` and surfaces any other key's refusal; gen-algebra publishes no `search` (the runner is retired); a same-name unmigrated pair, `closure` differing: unequal; rows 77/78                                                                                                                                                                                                                                                    |
| C58 -- an `addCheck`'d nixpkgs leaf is not its base        | 0034                     | `foreign-addcheck-leaf-does-not-unify`: through the hub's `modules.types.typeEq`, `addCheck str p` and `addCheck str q` compare unequal where both minted as `str` and compared equal; one binding, leaf or `listOf str`, still equals itself                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| C62 -- a check-only `mkOptionType` merges by the default   | 0025 item 1              | `mkoptiontype-default-merge`: a type stating `name` and `check` and no fold merges as nixpkgs' constructor default does, so `heddle` defined `[ "warp" ]` and `[ "weft" ]` in two files reads `[ "weft" "warp" ]`, where the pair was refused as unequal; `1` beside `2` is still refused catchably, and `{ a = 1; }` beside `{ a = 2; }` is refused where nixpkgs keeps the first file's value without a word                                                                                                                                                                                                                                          |
| C63 -- a dedup collapse keeps every dependency edge        | 0025 item 1, 0034        | `movement-dedup-store-path`: two strings naming `pkgs.hello`'s `.drv`, equal under `==` and differing in string context (`drvPath` and its `unsafeDiscardOutputDependency`), collapse under `byDatum` into one datum carrying the union of both contexts; 1 kept, 1 licensed drop, where the address used to abort (`… is not allowed to refer to a store path`)                                                                                                                                                                                                                                                                                        |
| C64 -- a kind mark over per-field regime tags              | 0034                     | `kind-mark-regime-tags`: two `selvage` kinds differing only in a refinement predicate mint one mark and `kindEq` refuses them by name; two differing only in `int` against `str`, a default, or a registered `between`'s bounds, mint apart and `kindEq` decides them; row 82. One `bolt` instance of each sealed-only pair carries one stamp, and `sel.entity`'s `selectorEq` refuses the two by name, as `kindEq` refuses their kinds; row 89. Two kinds whose field is typed by two constructions of one check-only `mkOptionType "tension"` carrying a `description` back-edge are refused by name, and one construction shared by both is one kind |
| C65 -- a foreign nested role crosses as carried            | 0014, 0025 item 1        | `foreign-nested-role-carried`: a foreign `bobbin` stating its element in `nestedTypes` beside a payload that is the element itself reads `carries.element` and `nestedTypes.elemType`, and `bobbin port` redeclared as `bobbin int` refuses 70000; `aspectsRoot` reads the same; the `bobbin str` twin still merges, where before both read `nestedTypes = { }` and the pair accepted 70000                                                                                                                                                                                                                                                             |
| C66 -- a self-referential node value, memoised             | 0008 §2, 0025            | `self-referential-node-value`: a chain whose node values each carry themselves goes through `genMemo.propagateEager` and reads the cold build's `[ 211 200 ]`; its hash is `null` (always-dirty) where plain values hash                                                                                                                                                                                                                                                                                                                                                                                                                                |
| C67 -- a crossing's import relatum is an identity          | 0016 ruling 4            | `crossing-import-relatum-identity`: through the hub's mint, two declarations of the import `bobbin` differing only in contract cross one supply at one loom as TWO crossings, the same declaration from two declarers as ONE, and no node's `import` is the name `bobbin`, which rides as its `name`                                                                                                                                                                                                                                                                                                                                                    |
| C68 -- a `mkType` kind publishes its option path           | 0025                     | `mktype-kind-publishes-option-path`: a `selvage` declared through `mkSchemaOption { mkType }` whose result omits `kind` reads `.kind` = `"selvage"` and `mkInstanceType` admits it, where `.kind` aborted uncatchably; a result echoing `"bobbin"` still reads `"selvage"`; a correct echo is the control                                                                                                                                                                                                                                                                                                                                               |
| C69 -- one function at a shared key agrees ×3              | 0025 item 1              | `mkoptiontype-shared-key-identity`: a check-only `mkOptionType` compares each definer's own value at a shared key, so `heddle = { a = spin; }` in two files, over one binding `spin`, reads `[ "a" ]` on Nix, Determinate and Lix, where Nix and Determinate refused and Lix kept it; `spin` passed through `specialArgs` reads the same; two closures of one lambda are still refused catchably                                                                                                                                                                                                                                                        |
| C70 -- a foreign vocabulary publishes its own names        | 0014, 0025 item 1        | `foreign-vocabulary-publishes`: gen-merge built over nixpkgs' `str`/`int`/`bool` alone publishes its namespace, and `spool` typed by that `str` reads `sateen` and refuses `1`, where the whole namespace refused because the vocabulary lacked `attrs`/`attrsOf`/`listOf`/`option`; the gen-types flake passed for its `lib` is refused catchably, and its `lib` is the control                                                                                                                                                                                                                                                                        |
| C71 -- a vocabulary collision refuses per name             | 0014, 0025 item 1        | `foreign-vocabulary-collides-per-name`: gen-merge built over nixpkgs' `str`/`nullOr` publishes every name but the undeclared `nullOr`: `spool` typed by that `str` reads `sateen`, gen-merge's `listOf` over it reads `[ "sateen" ]`, and `nullOr` refuses catchably, where the whole namespace refused at `nullOr`                                                                                                                                                                                                                                                                                                                                     |
| C72 -- a pkgs-bearing node value, memoised                 | 0008 §2, 0025 item 1     | `pkgs-bearing-node-value`: C66's chain with node values carrying `pkgs`, whose walk meets nixpkgs' lazy throws, goes through `genMemo.propagateEager` and reads the cold build's `[ 211 "hello" ]`; its hash is `null` (always-dirty) where plain values hash                                                                                                                                                                                                                                                                                                                                                                                           |
| C73 -- the nesting seam as a union member                  | 0014, 0025 item 1        | `nesting-seam-union-member`: `either spoolTree str` in gen-merge's own eval takes a function def as the tree, `spool = "sateen"`, and `"selvedge"` as the string, as nixpkgs' `either` over its own tree type does, where both were refused through the tree's `check`                                                                                                                                                                                                                                                                                                                                                                                  |
| C74 -- an addressed class crossing                         | 0028                     | `addressed-class-crossing`: `picot`'s `quilting` content, adapted by consumer code, is addressed `extraModules.smocking.picot` to the REAL `genDelivery.realize` and arrives at the smocking terminal only, the quilting and tatting terminals receiving nothing, where the node-keyed inlet reached all three; addressed where smocking does not realize (`ruche`), it is refused by name (T5 row94) where it was dropped                                                                                                                                                                                                                              |
| C75 -- a cross-instance nixpkgs type decides               | 0034, 0025 item 1        | `foreign-type-cross-instance-decides`: through the hub's `modules.types.typeEq`, `port`, `ints.between`, `nonEmptyStr`, `addCheck str` and `listOf port` from two nixpkgs lib instances compare unequal where the record `==` overflowed, and two records sharing every closure still separate on `description`                                                                                                                                                                                                                                                                                                                                         |
| C76 -- a leaf `freeformType` folds the undeclared plane    | 0025 item 1              | `freeform-leaf-type-folds`: `selvage = "twill"` reads `"twill"` under `freeformType = types.str` and under a bare `defineType` leaf, where the freeform fold aborted uncatchably; `selvage` and `bobbin` from two files are refused catchably under `str`, and the same two files read both keys under `attrsOf str`                                                                                                                                                                                                                                                                                                                                    |
| C77 -- one `lib` passed through `withArgs` twice merges ×3 | 0034, 0025 item 1        | `submodule-withargs-shared-lib`: the flake's own nixpkgs `lib`, reaching two declaring modules as a `specialArgs` formal, is handed by each to `(submodule [ reads ]).withArgs`, and the submodule reads `LIB-ARRIVED` on Nix, Determinate and Lix, where Nix and Determinate threw on a removed nixpkgs alias the comparison walked; `lib` against `builtins` is still refused catchably                                                                                                                                                                                                                                                               |
| C78 -- the schema option redeclared, one construction      | 0034, 0025 item 1        | `schema-option-redeclaration`: `mkSchemaOption { }` declared twice, one value or two calls, reads `selvage` through both declarations; two declarations differing in `strict` are refused catchably in both orders, and so are two whose facet option is typed by two constructions of one check-only `mkOptionType`, while one facet value shared by both merges                                                                                                                                                                                                                                                                                       |
| C79 -- same-named `mkOptionType`s, one construction        | 0034, 0025 item 1        | `mkoptiontype-samename-redeclaration`: two check-only `mkOptionType "tension"` declarations for one option are refused catchably in both orders, each on a value the later one accepts, where the later one decided in silence; so is a pair whose `description` carries a back-edge; one value declared twice reads its value and keeps its check; a `//` derivation of it is refused beside it                                                                                                                                                                                                                                                        |
| C80 -- a foreign type is not the type it is named after    | 0034                     | `foreign-same-name-type-does-not-unify`: through the hub's `modules.types.typeEq`, an `addCheck`'d `int` installed by `lib.extend`, a gen-merge `mkOptionType` named `int`, and `nonEmptyListOf str` compare unequal to the type whose name they carry; a type still equals itself, a shared binding and its `// { }` copy                                                                                                                                                                                                                                                                                                                              |
| C81 -- the `self.node` seam guard, both demand paths       | 0008, 0030               | `codomain-seam-guard`: `damask` reads its sibling `faille`'s `ply` through `self.node`, once in a synthesized body and once in a circular attribute's step; under a declared relation carrying the edge both read `2`, under the empty relation both are refused, and gen-scope's reason names `damask`, `faille` and the relation `[]`; `damask`'s read of its own record is not refused                                                                                                                                                                                                                                                               |
| C82 -- a check-overridden `freeformType` merges raw        | 0025 item 1              | `freeform-check-override-accepted`: `picks = 1` reads `{ picks = 1; }` under `freeformType` nixpkgs `attrsOf int // { check = _: false; }` (v2) and `attrs // { check = _: false; }`, as nixpkgs folds by the raw `merge`, where gen-merge refused both; the same two types at a declared option are still refused, beside the stock types reading it                                                                                                                                                                                                                                                                                                   |
| C83 -- the schema option's introspection, redeclared       | 0034, 0012, 0025 item 1  | `schema-option-redeclared-reads-its-kinds`: one `mkSchemaOption { }` value declared by two modules reads `_kindNames` as `[ "bobbin" "selvage" ]`, and its `_kindNames`, `_topology` and `_edges` equal the option declared once; before, the introspection module was imported twice and `_kindNames` refused "read-only, but it is defined 2 times"                                                                                                                                                                                                                                                                                                   |
| C84 -- a schema's containment, read off gen-graph          | 0012                     | `schema-containment-topology`: `damask` contains `faille`, which contains `picot` and `gusset`; `grosgrain` stands alone. `_roots` read `[ "damask" "grosgrain" ]`, `_leaves` `[ "grosgrain" "gusset" "picot" ]`, `picot`'s parent `faille`, and `faille`'s children `[ "gusset" "picot" ]` in kind-name order                                                                                                                                                                                                                                                                                                                                          |
| C85 -- the `__` keys crossing between libraries            | 0014                     | `dunder-contracts`: `hemline/placket`'s projected data carries `[ "__identity" ]`, `keyRef "mill/stitch"` carries `[ "__keyRef" ]`, the `selvage` kind value carries `[ "__functor" "__mint" "__sealed" ]` and its kind selector `[ "__sel" ]`: exactly the keys their owners state as contracts                                                                                                                                                                                                                                                                                                                                                        |

...
| C64 -- a kind mark over per-field regime tags             | 0034                     | `kind-mark-regime-tags`: two `selvage` kinds differing only in a refinement predicate mint one mark and `kindEq` refuses them by name; two differing only in `int` against `str`, a default, or a registered `between`'s bounds, mint apart and `kindEq` decides them; row 82                                                                                                                                                          |
